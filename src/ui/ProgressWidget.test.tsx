...
describe('ProgressWidget', () => {
  it('shows Level 1 at 0 XP', () => {
    render(<ProgressWidget progress={makeProgress(0)} />);
    expect(screen.getByText('Level 1')).toBeInTheDocument();
  });

  it('shows Level 2 at 100 XP', () => {
    render(<ProgressWidget progress={makeProgress(100)} />);
    expect(screen.getByText('Level 2')).toBeInTheDocument();
  });

  it('shows total XP', () => {
    render(<ProgressWidget progress={makeProgress(150)} />);
  ...
  it('shows "MAX" at max level (500 XP)', () => {
    render(<ProgressWidget progress={makeProgress(500)} />);
    expect(screen.getByText('MAX')).toBeInTheDocument();
  });

  it('is expanded by default', () => {
    const { container } = render(<ProgressWidget progress={makeProgress(0)} />);
